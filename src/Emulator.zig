...
pub fn execute_instruction(self: *Self, inst: Instruction) void {
    switch (inst) {
        .add => |*val| {
            self.setreg(
                val.rd,
                self.getreg(val.rs1) +% self.getreg(val.rs2)
            );
        },
        .addi => |*val| {
            self.setreg(
                val.rd,
                self.getreg(val.rs1) +% @bitCast(u64, @intCast(i64, val.imm))
            );
        },
        .and_ => |*val| {
            self.setreg(
        ...
        .auipc => |*val| {
            self.setreg(
                val.rd,
                (self.pc -% 4) +% @bitCast(u64, @intCast(i64, val.imm))
            );
        },
        .beq => |*val| {
        ...
        .lhu => |*val| {
            const address = self.getreg(val.rs1) +% @bitCast(u64, @intCast(i64, val.imm));
            const value = @intCast(u64, self.memory.items[address]) | (@intCast(u64, self.memory.items[address + 1]) << 8);
            self.setreg(val.rd, value);
        },
        .lui => |*val| {
        ...
        .slt => |*val| {
            self.setreg(
                val.rd,
                @boolToInt(@bitCast(i64, self.getreg(val.rs1)) < @bitCast(i64, self.getreg(val.rs2)))
            );
        },
        .slti => |*val| {
            self.setreg(
                val.rd,
                @boolToInt(@bitCast(i64, self.getreg(val.rs1)) < @intCast(i64, val.imm))
            );
        },
        .sltiu => |*val| {
            self.setreg(
                val.rd,
                @boolToInt(self.getreg(val.rs1) < @bitCast(u64, @intCast(i64, val.imm)))
            );
        },
        .sltu => |*val| {
            self.setreg(
                val.rd,
                @boolToInt(self.getreg(val.rs1) < self.getreg(val.rs2))
            );
        },
        .sra => |*val| {
            const shamt = self.getreg(val.rs2) & 0b11111;
